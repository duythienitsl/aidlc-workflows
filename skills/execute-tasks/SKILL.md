...
2a. **Create the working branch** (once per initiative, before the first task) — Branch from the **`dev`** branch of the repo that holds the code being changed (for backend work this is the **`api-platform`** repo, not the `LH-v1` root). Name the branch `<type>/LHP-<ticket>`:
   - `<type>` = **`bug`** when the Jira issue type is **Bug**, otherwise **`task`** (features/tasks/improvements).
   - Example: Bug LHP-2186 → `bug/LHP-2186`; Task LHP-2184 → `task/LHP-2184`.
   - Ensure `dev` is clean and up to date with `origin/dev` first, then `git checkout dev && git pull` and `git checkout -b <type>/LHP-<ticket>`. If the branch already exists, check it out instead of recreating it.
